...
class AutoAccessibilityService : AccessibilityService() {
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            taskExecutor.taskResult.collect { result ->
                ...
                // 这里可以发送广播通知UI更新
                sendTaskStatusBroadcast(result.status, result.message)
            }
        }
    }
     ...
     * 发送任务状态广播
     */
    private fun sendTaskStatusBroadcast(status: TaskStatus, message: String) {
        val intent = Intent("com.xmbest.autask.TASK_STATUS_CHANGED").apply {
